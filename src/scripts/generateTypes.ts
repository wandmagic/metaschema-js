...
function transformSchema(schema: Schema): Schema {
    function processObject(obj: any): any {
        ...
        if (obj.$id && typeof obj.$id === 'string') {
            obj.$id = obj.$id.replace(/#(assembly_)?(field_)?(metaschema-complete-?)?(metaschema-[a-zA-Z-]+[_:])?/, '#');
        }

        // Process $ref
        if (obj.$ref && typeof obj.$ref === 'string') {
            obj.$ref = obj.$ref.replace(/#(assembly_)?(field_)?(metaschema-complete-?)?(metaschema-[a-zA-Z-]+[_:])?/, '#');
        ...
        for (let key in obj) {
            const newKey = key.replace(/^(field_)?(metaschema-complete-)?(metaschema-[a-zA-Z-]+[_:])?/, '');
        ...
        schema.oneOf = schema.oneOf.map(item => {
            ...
            for (let key in properties) {
                if (key === '$schema') {
                    ...
                    const newKey = key.replace(/^(field_)?(metaschema-complete-)?(metaschema-[a-zA-Z-]+:)?/, '');
                    newProperties[newKey] = properties[key];
                }
            }
            item.properties = newProperties;
            if (item.required) {
                item.required = item.required.map(req => req.replace(/^(field_)?(metaschema-complete-)?(metaschema-[a-zA-Z-]+:)?/, ''));
            }
            return item;
        });
    }
    ...
    const updatedSchemaString = schemaString
        .replace(/#(assembly_)?(field_)?(metaschema-complete-?)?(metaschema-[a-zA-Z-]+[_:])?/g, '#')
        .replace(/(?<!^|[{,]"\s*)(field_)?(metaschema-complete-)?(metaschema-[a-zA-Z-]+[_:])?/g, '')
...
fs.readFile('./src/schema/metaschema.complete.json', 'utf8')
    .then(JSON.parse)
    // .then(findConflictingDefinitions)
    // .then(transformSchema)
    // .then(ts => {
    //     return fs.writeFile('metaschema.complete.json', JSON.stringify(ts, null, 2))
    //         .then(() => ts);
    // })
    .then(schema => compile(schema, 'METASCHEMA', { bannerComment: '' }))
